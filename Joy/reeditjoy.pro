...
;loadct, 0, /sil
;plot_imag, magscl(narmap.data), background = 255, color = 0;
;setcolors, /sil, /sys
...
if poslat && neglat gt 0 then begin
	...
	if poslon gt neglon then begin
		print, 'Leading spot is positive.'
		leadinglat=poslat
		leadinglon=poslon
		trailinglat=neglat
		trailinglon=neglon
	endif else begin 
		print, 'Leading spot is negative.
		leadinglat=neglat
		leadinglon=neglon
		trailinglat=poslat
		trailinglon=poslon
	endelse
	tanT = (trailinglat-leadinglat)/(trailinglon-leadinglon)
	T = 180/!pi*atan(tanT)
...
openu, lun, "reeditjoy.dat", /GET_LUN, /APPEND
